...
function DiseaseCard({ disease }: DiseaseCardProps) {
  ...
  const iconTone =
    disease.category.toLowerCase() === "infectious"
      ? "from-rose-500/15 to-orange-400/15"
      : disease.category.toLowerCase() === "chronic"
        ? "from-cyan-500/15 to-teal-400/15"
        : "from-violet-500/15 to-fuchsia-400/15";
    ...
    <GlassCard className="flex h-full flex-col p-6 transition duration-300 hover:-translate-y-1 hover:shadow-[0_24px_60px_rgba(15,23,42,0.10)]">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-4">
          ...
          <div>
            <p className="text-xs font-medium uppercase tracking-[0.18em] text-cyan-800">{disease.body_system}</p>
        ...
        <div>
          ...
          <div className="mt-2 flex min-h-12 flex-wrap gap-2">
            {symptomPreview.map((symptom) => (
              <span key={symptom} className="rounded-full border border-cyan-100 bg-gradient-to-r from-cyan-50 to-sky-50 px-3 py-1 text-xs text-cyan-800">
                {symptom}
              </span>
            ))}
          </div>
